 ...
 */
 void iqs_swap(int* data, int i, int j) {
    int temp = data[i];
    data[i] = data[j];
    data[j] = temp;
}
 ...
 */
 int iqs_partition (int* data, int left, int right) {
    int x = data[left];
    int i = left-1;
    int j = right+1;
    while (TRUE) {
 ...
 */
void prints_ia (int size, int* data) {
    int i;
    for (i=0; i<size; ++i)   printf ("%i ", data[i]);
    printf ("\n");
}
 ...
 * @param j         Second index into data.
 */
 void idqs_swap(double* data, int* ndx, int i, int j) {
    int temp = ndx[i];
    ndx[i] = ndx[j];
    ndx[j] = temp;
}
 ...
 * @return             Some middle value to split over
 */
 int idqs_partition (double* data, int* ndx, int left, int right) {
 ...
 */
 void print_ida (int size, double* data, int* ndx) {
    int i;
    for (i=0; i<size; ++i)   printf ("(%i, %f) ", ndx[i], data[ndx[i]]);
    printf ("\n");
}

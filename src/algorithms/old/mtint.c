...
double gennormal (void) {
  double x1 = genuniform();
  double x2 = genuniform();
...
genuniform(void) 
{
  ...
  double val = 0.0;
  do {
    unsigned long l = genrand();
    int i = l &= 0x000fffff;
    val = (double)(i/((double)((int)(0x000fffff))));
  } while (val == 1.0);
  ...
  //double val;
  //unsigned int e = 1022;
  //unsigned long *longword = (unsigned long *) (&val);

  //longword[0] = genrand();
  //longword[1] = genrand();
  
  //longword[1] &= 0x000fffff;

  //while (e > 0) {
  //unsigned long it = genrand();
  //int i;
    
  //for (i=0; i<31; i++) {
  //if (it & 0x00000001) { it >>= 1; e--; }
  //else {
  //longword[1] |= e << 20;          // fixing the exponent.
